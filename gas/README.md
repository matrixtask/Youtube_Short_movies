...
毎朝Slackに「撮影カード」と別の「制作メモ」を届け、動画の受付・承認・
...
- 朝、Slackに台本が届く → スマホを横にして、1問ずつ【読む】【戻って読む】の引用部分を読む
- 自分の考えは任意。支持・反論・保留・別案を追加でき、省略しても完成。制作メモは全カードの後に別送される
- **撮った動画を台本のスレッドにそのまま投稿** → 編集キューに登録され、
  台本と自動で紐づく。**できあがったショートが同じスレッドに返ってくる**
  （チャンネル直下に投稿した場合は最新の台本に紐づきます）
- 台本と関係ない動画（雑談・思いつき）は「**フリートーク**」と一言添えて
  投稿すれば台本と紐づけず、話のまとまりごとに切り出されます
...
- 「台本 状態」で生成中の段階を確認、「台本 再開 <ジョブID>」で失敗した段階から再開できる
- 生成は1分ワーカーで段階ごとに保存して進む。ScriptJobsシートとトリガーは初回受付時に自動作成。詳細は [分割実行と再開](../SCRIPT_JOBS.md)
...
毎週月曜、撮影実績からテーマの重みが自動調整されます（`tuneThemeWeights`）:

- **動画が来なかった台本**・**動画は来たがショートが生まれなかった質問**の
  テーマは miss として記録され、missが続くと重みが半減 → 0.5未満で 0（停止）
- ショートが生まれたテーマは hit として記録され、hitが続くと +0.5（上限3）
- 累計は `hits` / `misses` 列に残り、調整結果は週次サマリーでSlackに届く
- 止まったテーマを復活させたいときは、シートで weight を手で戻すだけ

「喋りづらいテーマが自然に減り、喋れるテーマが残る」仕組みです。
