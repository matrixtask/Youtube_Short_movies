...
[台本生成基準](SCRIPT_GUIDE.md)に従います。共通の前提・二つの仮説・見直し条件・締めを
読むカードにし、本人の意見は任意、制作メモは別表示にします。両層を既存のhint列へ保存し、
Slackと編集処理へ渡します。直近30問を参照し、レイ・セバスチャン・ハンニバルの会議と
ミアの批評を経て生成。各段階をScriptJobsへ保存し、1分トリガーの別実行で続行します。
執筆の構造不良も次の実行で1回だけ修復します。「台本 状態」で進行確認、
「台本 再開 <ジョブID>」で停止段階から再開できます。詳しくは [分割実行と再開](SCRIPT_JOBS.md)。
...
通常の生成は段階ごとに保存し、別のワーカー実行へ引き継ぎます。全体で240秒以内に収める必要はありません。
1回のAPIがGASの実行上限を超える場合は停止検知後に再開が必要です。詳細は [SCRIPT_JOBS.md](SCRIPT_JOBS.md)。
