...
| いつ使う | 方法 |
|---|---|
| 毎回適用したい | `config.yaml` を編集してコミット。口調は `style_notes`、テロップ・挿絵・映像の**位置やサイズ**は `caption_margin` / `illustration_y` 等（コメント参照） |
| この動画だけ | **Slackで動画に添えるメッセージがそのまま編集指示になる**（例:「テロップ多め、ツッコミ辛口で」「挿絵なしで」）。「フリートーク」もここに書く |
| 1カ所だけ直したい | `workspace/sessions/<動画名>/plan.json` を編集 → 該当mp4を消して `make run`（字幕文言・カット位置・スコアを直接いじれる） |
